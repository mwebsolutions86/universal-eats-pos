...
import LiveOrdersTab from './components/LiveOrdersTab'; // Import ajouté
...
const App = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // On laisse passer en mode dégradé si besoin, ou on bloque ici.
        setConfigLoaded(true); 
      }
    };
    init();
  ...
  const handleOpenSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStep('app'); // On passe à l'application principale
      setCurrentView('pos'); // Vue par défaut
    } catch (err) {
  ...
  const handleLogout = () => {
    // Ici on pourrait ajouter une logique de clôture de session si nécessaire
    setUser(null);
    setSession(null);
    setStep('login');
    setPin('');
  ...
  // --- RENDU DU CONTENU PRINCIPAL (DANS LE LAYOUT) ---
  const renderView = () => {
    switch (currentView) {
        case 'pos':
            return user && session ? (
            ...
            case 'live-orders': // ou l'ID que tu as mis dans Sidebar.tsx (ex: 'orders')
  ...
  // 2. MAIN APPLICATION (AUTHENTIFIÉ)
  if (step === 'app' && user) {
    return (
      <MainLayout
        user={user}
  ...
  // 3. ÉCRAN OUVERTURE CAISSE (MODAL STYLE)

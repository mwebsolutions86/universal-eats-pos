...
export const renderOrderItemDetails = (options: any) => {
  ...
  if (typeof options === 'string') { try { parsedOpts = JSON.parse(options); } catch (e) { return null; } }
  ...
  if (parsedOpts.variation) {
      itemsRender.push(<span key="var" className="block text-xs font-black text-black dark:text-white uppercase tracking-wide mt-0.5">📏 {parsedOpts.variation.name}</span>);
  }
  ...
  if (Array.isArray(supps) && supps.length > 0) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      supps.forEach((opt: any, i: number) => {
  ...
  // C. Exclusions
  const removed = parsedOpts.removed_ingredients || [];
  ...
  // D. Note
  const note = parsedOpts.note || (typeof parsedOpts === 'object' && !Array.isArray(parsedOpts) ? parsedOpts.comment : null);
...
export const getStatusBadge = (status: string) => {
  ...
  return <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${styles[status] || 'bg-gray-100 text-gray-800'}`}>{labels[status] || status}</span>;
};
...
export const OrderCard: React.FC<OrderCardProps> = ({ order, onClick }) => {
    return (
        <div onClick={() => onClick(order)} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-3 shadow-sm hover:shadow-md hover:border-primary/50 transition-all cursor-pointer flex flex-col h-auto min-h-[140px]">
            <div className="flex justify-between items-start mb-2"><div><span className="font-mono font-black text-base text-slate-800 dark:text-white">#{order.order_number}</span><div className="text-[10px] text-slate-400 flex items-center gap-1"><Clock size={10} />{new Date(order.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div></div>{getStatusBadge(order.status)}</div>
            <div className="flex-1 mb-2">
                ...
                <div className="space-y-1.5">
                    {order.items?.slice(0, 3).map((item: OrderItem, idx: number) => (<div key={idx} className="flex flex-col text-xs leading-tight"><span className="font-bold text-slate-700 dark:text-slate-200">{item.quantity}x {item.product_name}</span>{renderOrderItemDetails(item.options)}</div>))}
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
          {orders.map((order) => (
            <tr key={order.id} onClick={() => onOrderClick(order)} className="hover:bg-blue-50 dark:hover:bg-slate-800 cursor-pointer transition-colors group">
              ...
              <td className="p-4"><div className="space-y-2">{order.items?.slice(0, 3).map((item: OrderItem, idx: number) => (<div key={idx} className="leading-tight"><div className="flex items-baseline gap-2"><span className="font-bold text-slate-800 dark:text-slate-200">{item.quantity}x {item.product_name}</span></div>{renderOrderItemDetails(item.options)}</div>))}{order.items && order.items.length > 3 && (<span className="text-xs text-slate-400 italic">... et {order.items.length - 3} autres</span>)}</div></td>
              ...
              <td className="p-4 text-center">{getStatusBadge(order.status)}<div className="text-[10px] text-slate-400 mt-1 flex items-center justify-center gap-1"><Clock size={10} />{new Date(order.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div></td>
            </tr>
          ))}
        </tbody>
      </table>
